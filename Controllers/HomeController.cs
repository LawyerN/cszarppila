...
namespace FootballScoreApp.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string sortBy = "goals")
        {
            ...
            ViewBag.AllScorers = sortBy == "assists" 
            ...
            ViewBag.AllStadiumStats = await _context.Matches
                .Include(m => m.Stadium)
            ...
            ViewBag.TopStadiumStats = await _context.Matches
                .Include(m => m.Stadium)
                .Where(m => m.IsCompleted)
                .GroupBy(m => new { m.Stadium!.Id, m.Stadium.Name })
                .Select(g => new {
            ...
            ViewBag.TopRefereeStats = await _context.Matches
                .Include(m => m.Referee)
            ...
            ViewBag.AllRefereeStats = await _context.Matches
                .Include(m => m.Referee)
                .Where(m => m.RefereeId != null)
                .GroupBy(m => new { m.Referee!.Id, m.Referee.FirstName, m.Referee.LastName })
                .Select(g => new {
